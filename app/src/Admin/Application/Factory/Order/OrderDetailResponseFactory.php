...
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailAddressResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailContactResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailDeliveryAddressResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailInformationResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailInvoiceAddressResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailItemResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailItemsResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailPaymentItemResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailPaymentResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailShippingResponse;
use App\Admin\Application\DTO\Response\Order\OrderDetail\OrderDetailSummaryResponse;
...
final readonly class OrderDetailResponseFactory
{
    ...
    {
        $productTotalPrice = $order->getTotalPrice() === null ? null : $this->moneyFactory->create($order->getTotalPrice());
        $carrier = $order->getCarrier() === null ? null : $this->moneyFactory->create($order->getCarrier()->getFee());
        $paymentMethod = $order->getPaymentMethod() === null ? null : $this->moneyFactory->create($order->getPaymentMethod()->getFee());

        $totalPrice = null;
        if ($productTotalPrice !== null) {
            $totalPrice = $productTotalPrice;
            if ($carrier !== null) {
                $totalPrice = $totalPrice->add($carrier);
            }
            if ($paymentMethod !== null) {
                $totalPrice = $totalPrice->add($paymentMethod);
            }
        }

        return new OrderDetailSummaryResponse(
            summaryProductPrice: $productTotalPrice?->getFormattedAmountWithSymbol(),
            deliveryFee: $carrier?->getFormattedAmountWithSymbol(),
            paymentMethodFee: $paymentMethod?->getFormattedAmountWithSymbol(),
            totalPrice: $totalPrice?->getFormattedAmountWithSymbol(),
        );
    }
}
