    ...
    useEffect(() => {
        if (settingValue && settingValue?.type === 'select') {
            ...
            const isEmpty =
                ...
                (typeof selectedParse === 'object' &&
                    !Array.isArray(selectedParse) &&
                    Object.keys(selectedParse).length === 0);

            if (!isEmpty) {
                setSelectedOption(
                    settingValue?.value.find((element) => element.value === Number(selectedParse.id)).value,
                );
            }
        }
