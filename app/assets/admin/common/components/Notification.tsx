...
const Notification: React.FC<AppNotificationProps> = ({ label, variant, time }) => {
    ...
    const iconVariants: Record<NotificationType, JSX.Element> = {
    ...
    return (
        <div className="py-5">
            <div className="w-full flex items-center justify-between gap-3 max-w-[400px] rounded-lg px-3 py-4 shadow-xl bg-white relative">
                ...
                <div
                    ...
                    style={{ '--progress-time': `${time}ms` } as React.CSSProperties}
                />
            </div>
        </div>
    );
