...
const TableToolbar: React.FC<TableToolbarProps> = ({
    ...
    const filtersContent = () => {
        ...
        return additionalFilters.map((filterComponent, index) => (
            <div key={index}>{filterComponent}</div>
        ));
    };
    ...
    const renderModal = () => (
        <>
            ...
            <ModalBody>
                <div className="flex flex-col gap-[2rem] justify-between lg:w-[500px]">
                    {filtersContent()}
                </div>
            </ModalBody>
        </>
    ...
    const openFilters = () => {
        openModal(renderModal(), ModalPositionType.RIGHT);
    };
    ...
    return (
        <>
            {additionalToolbarContent && (
                <div className="mb-[2rem]">{additionalToolbarContent}</div>
            )}
